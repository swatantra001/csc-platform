...
const THEMES = {
  ...
  dark: {
    ...
    docIconColor: "#f87171",
    docIconColorDark: "#f87171",
    btnDangerBgDark: "#dc2626", // badk in dark mode we use a lighter red for danger buttons to maintain contrast and avoid overwhelming the user
  },
} as const;
...
export default function UserProfilePage() {
  ...
  const handleSaveMobile = async () => {
    ...
    const result = await updateUserProfile({ mobile: val });
    if (result.success) {
      setUser((u) => ({ ...u, mobile: val }));
      setEditingMobile(false);

    } else {
      setMobileError(result.error || "Failed to update mobile number");
    }
  };
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {editingMobile ? (
                        <>
                          ...
                          {mobileError && <span style={{ color: !isDark ? T.btnDangerBg : T.btnDangerBgDark, fontSize: 12, marginTop: 6 }}>{mobileError}</span>}
                        </>
                      ) : (
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4 }}>
