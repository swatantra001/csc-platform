...
// export default function SemanticBot({ post, color }: { post: DbPost; color: string }) {
...
//           /* webpackIgnore: true */ 
// 		  // @ts-ignore
//           "https://cdn.jsdelivr.net/npm/@xenova/transformers@2.17.2" // @ts-ignore
//         );
        
//         env.allowLocalModels = false;

//         // Xenova/all-MiniLM-L6-v2 is actually fantastic for this once fed the right data
...
//         // ✨ THE FIX: We are now feeding ALL links, instructions, and FAQs into the AI's brain!
//         const rawFacts = [
...
//         // ✨ THE FIX: Lowered strictness from 0.35 to 0.25 so short questions like "apply link" work perfectly!
...
//           style={{ position: "fixed", bottom: 24, right: 24, width: 60, height: 60, borderRadius: "50%", background: color, color: "#fff", border: "none", boxShadow: "0 8px 24px rgba(0,0,0,0.2)", cursor: "pointer", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}
//         >
//           🤖
//         </button>
//       )}
...
//       {isOpen && (
//         <div style={{ position: "fixed", bottom: 24, right: 24, width: 340, height: 500, background: "#fff", borderRadius: 16, boxShadow: "0 12px 40px rgba(0,0,0,0.2)", zIndex: 1000, display: "flex", flexDirection: "column", border: "1px solid #e2e8f0" }}>
          
//           <div style={{ background: color, color: "#fff", padding: "16px", display: "flex", justifyContent: "space-between", alignItems: "center", fontWeight: 700 }}>
//             <div>🤖 CSC AI</div>
...
//                 <div style={{ maxWidth: "85%", padding: "10px 14px", borderRadius: 12, background: m.role === "user" ? color : "#fff", color: m.role === "user" ? "#fff" : "#1e293b", border: m.role === "user" ? "none" : "1px solid #e2e8f0", fontSize: "0.85rem", whiteSpace: "pre-wrap" }}>
//                   {m.text}
//                 </div>
//               </div>
//             ))}
...
//                   <button key={idx} onClick={() => handleSend(q)} style={{ background: "rgba(0,0,0,0.05)", border: "none", color: color, fontSize: "0.75rem", padding: "6px 12px", borderRadius: 20, cursor: "pointer", fontWeight: 600 }}>
//                     {q}
//                   </button>
//                 ))}
//               </div>
...
//               disabled={!isReady}
//               style={{ flex: 1, padding: "10px 14px", border: "1px solid #e2e8f0", borderRadius: 20, outline: "none", fontSize: "0.85rem", background: "#f1f5f9" }}
//             />
//             <button 
//               onClick={() => handleSend(input)}
//               disabled={!input.trim() || !isReady}
//               style={{ width: 40, height: 40, borderRadius: "50%", background: input.trim() && isReady ? color : "#94a3b8", color: "#fff", border: "none", cursor: input.trim() && isReady ? "pointer" : "not-allowed" }}
//             >
//               ➤
//             </button>
//           </div>
