// "use client";
// // app/status/page.tsx
// // Public Status Tracker — no login required
// // Design: Post office tracking board — navy + cream, stamp trail, dignified

// import { useState, useEffect, useRef } from "react";
// import { getPublicRequestStatusAction } from "@/app/actions/status";
// import dynamic from "next/dynamic";

// // Dynamically import the map so SSR doesn't crash
// const LiveDeliveryMap = dynamic(() => import("@/components/LiveDeliveryMap"), {
//   ssr: false,
//   loading: () => <div className="h-[400px] w-full bg-slate-100 animate-pulse rounded-2xl mt-6 border border-slate-200 flex items-center justify-center text-slate-400 font-bold">Loading Map Engine...</div>
// });
...
//   delivery_status?: string; // ✨ Add this!
...
// const STATUS_CFG: Record<StatusKey, { color: string; bgLight: string; bgDark: string; borderLight: string; borderDark: string; icon: string }> = {
//   submitted: { color: "#6a6259", bgLight: "#f5f1e8", bgDark: "#1a1810", borderLight: "#d8d0c0", borderDark: "#3a3228", icon: "📤" },
//   seen: { color: "#1a5aa0", bgLight: "#e8f0fb", bgDark: "#080f1f", borderLight: "#90b8e8", borderDark: "#0a2a5c", icon: "👁️" },
//   processing: { color: "#b07000", bgLight: "#fff8e8", bgDark: "#1a1200", borderLight: "#e8c860", borderDark: "#5c4000", icon: "⚙️" },
//   done: { color: "#1a7a3a", bgLight: "#e8f8ee", bgDark: "#081a0f", borderLight: "#80c890", borderDark: "#1a5c2a", icon: "✅" },
//   payment_pending: { color: "#c03010", bgLight: "#fff0e8", bgDark: "#1a0800", borderLight: "#e89070", borderDark: "#5c2010", icon: "💳" },
// };
...
//   // Palette
//   const bg = dark ? "#0c0e12" : "#f2f0e8";
//   const paper = dark ? "#141820" : "#ffffff";
//   const ink = dark ? "#e0ddd0" : "#1a1814";
//   const inkMid = dark ? "#8a8880" : "#6b6259";
//   const inkLt = dark ? "#4a4840" : "#a8a098";
//   const bord = dark ? "#252830" : "#ddd8cc";
//   const navy = dark ? "#1e2840" : "#1a3a5c";
//   const accent = "#c45c1a";

//   return (
//     <div style={{ background: bg, color: ink, minHeight: "100vh", fontFamily: "'Noto Sans', 'Noto Sans Devanagari', sans-serif" }}>
//       <style>{`
//         @import url('https://fonts.googleapis.com/css2?family=Noto+Serif+Devanagari:wght@700;900&family=Lora:wght@400;600;700&family=Noto+Sans:wght@400;500;600&family=Noto+Sans+Devanagari:wght@400;500;600&display=swap');
...
//         .search-input{width:100%;padding:14px 18px 14px 48px;border:2px solid ${bord};border-radius:8px;background:${paper};color:${ink};font-size:15px;font-family:inherit;outline:none;transition:border-color 0.2s;}
//         .search-input:focus{border-color:${accent};}
//         .search-input::placeholder{color:${inkLt};}
//         .search-btn{padding:14px 28px;background:${navy};border:none;border-radius:8px;color:#e8dfc8;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;transition:background 0.15s;white-space:nowrap;display:flex;align-items:center;gap:8px;}
//         .search-btn:hover{background:${dark ? "#2a3a5c" : "#243a5c"};}
//         .search-btn:disabled{background:${bord};color:${inkLt};cursor:not-allowed;}
//         .serif{font-family:'Noto Serif Devanagari','Lora',serif;}
//         .nav-link{font-size:13px;color:${inkMid};text-decoration:none;}
//         .nav-link:hover{color:${accent};}
//         .sample-chip{padding:6px 14px;background:${paper};border:1px solid ${bord};border-radius:20px;font-size:12px;color:${inkMid};cursor:pointer;font-family:inherit;transition:all 0.15s;}
//         .sample-chip:hover{border-color:${accent};color:${accent};}
//       `}</style>

//       {/* ── NAV ── */}
//       <nav style={{ background: paper, borderBottom: `1px solid ${bord}`, padding: "0 24px", height: 52, display: "flex", alignItems: "center", gap: 16 }}>
//         <a href="/" style={{ fontFamily: "'Noto Serif Devanagari', serif", fontSize: 16, fontWeight: 700, color: accent, textDecoration: "none" }}>
//           जन सेवा केंद्र
//         </a>
//         <span style={{ color: inkLt, fontSize: 13 }}>/</span>
//         <span style={{ fontSize: 13, color: inkMid }}>{t.pageTitle}</span>
//         <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
//           <button onClick={() => setLang(lang === "hi" ? "en" : "hi")} style={{ background: dark ? "#252830" : "#f0ece0", border: `1px solid ${bord}`, borderRadius: 20, padding: "4px 14px", fontSize: 11, color: inkMid, cursor: "pointer", fontWeight: 700, fontFamily: "inherit" }}>
//             {lang === "hi" ? "EN" : "हि"}
//           </button>
//           <button onClick={() => setDark(!dark)} style={{ background: dark ? "#252830" : "#f0ece0", border: `1px solid ${bord}`, borderRadius: 20, width: 32, height: 32, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
//             {dark ? "☀️" : "🌙"}
//           </button>
//         </div>
//       </nav>
...
//           <div style={{ width: 64, height: 64, borderRadius: "50%", border: `3px solid ${navy}`, margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, background: dark ? "#1e2840" : "#e8f0fb" }}>
//             🏛️
//           </div>
//           <h1 className="serif" style={{ fontSize: 28, fontWeight: 900, color: ink, marginBottom: 10, lineHeight: 1.2 }}>
//             {t.pageTitle}
//           </h1>
//           <p style={{ fontSize: 14, color: inkMid, lineHeight: 1.7, maxWidth: 480, margin: "0 auto" }}>{t.pageSub}</p>
//         </div>

//         {/* Search box */}
//         <div style={{ background: paper, border: `1px solid ${bord}`, borderRadius: 12, padding: "24px", marginBottom: 24, animation: "fadeIn 0.4s ease 0.1s both" }}>
//           <label style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: inkMid, display: "block", marginBottom: 10 }}>
//             {t.inputLabel}
//           </label>
//           <div style={{ display: "flex", gap: 10 }}>
//             <div style={{ position: "relative", flex: 1 }}>
//               <span style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", fontSize: 16, color: inkLt }}>🔍</span>
//               <input
//                 ref={inputRef}
...
//             <span style={{ fontSize: 11, color: inkLt }}>{lang === "hi" ? "उदाहरण:" : "Try:"}</span>
//             {["REQ-2025-001", "REQ-2025-002", "REQ-2025-003"].map((s) => (
//               <button key={s} className="sample-chip" onClick={() => { setQuery(s); handleSearch(s); }}>
//                 {s}
//               </button>
//             ))}
//           </div>
...
//               return (
//                 <div style={{ background: dark ? sc.bgDark : sc.bgLight, border: `1px solid ${dark ? sc.borderDark : sc.borderLight}`, borderRadius: 12, padding: "20px 24px", marginBottom: 20, display: "flex", gap: 16, alignItems: "center" }}>
//                   <div style={{ fontSize: 40, animation: "stampIn 0.4s ease" }}>{sc.icon}</div>
//                   <div>
//                     <div style={{ fontSize: 12, color: inkMid, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 4 }}>
//                       {t.pageTitle}
//                     </div>
//                     <div className="serif" style={{ fontSize: 22, fontWeight: 900, color: sc.color, lineHeight: 1 }}>
//                       {t.statusLabels[result.status]}
//                     </div>
//                     <div style={{ fontSize: 12, color: inkMid, marginTop: 4 }}>
...
//             {/* Request details */}
//             <div style={{ background: paper, border: `1px solid ${bord}`, borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}>
...
//                   return (
//                     <div key={label}>
//                       <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: inkLt, marginBottom: 4 }}>{label}</div>
//                       <div style={{ fontSize: 13, color: ink, fontWeight: 600, fontFamily: mono ? "'IBM Plex Mono', monospace" : "inherit" }}>{value}</div>
//                     </div>
//                   );
//                 })}
//               </div>
...
//               <div style={{ background: dark ? "#1a0800" : "#fff5f0", border: `1.5px solid ${dark ? "#8a3010" : "#f0a080"}`, borderRadius: 10, padding: "16px 20px", marginBottom: 20, display: "flex", gap: 14, alignItems: "center" }}>
//                 <span style={{ fontSize: 28, flexShrink: 0 }}>💳</span>
//                 <div style={{ flex: 1 }}>
//                   <div style={{ fontWeight: 700, fontSize: 14, color: dark ? "#ff8060" : "#c03010", marginBottom: 4 }}>
//                     {t.paymentDue} — ₹{result.paymentAmount}
//                   </div>
//                   <div style={{ fontSize: 12, color: inkMid }}>{t.payNote}</div>
//                 </div>
//                 <button style={{ background: accent, border: "none", borderRadius: 8, color: "#fff", padding: "10px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>
//                   {t.payNow} ₹{result.paymentAmount}
//                 </button>
//               </div>
//             )}
...
//             {/* Timeline */}
//             <div style={{ background: paper, border: `1px solid ${bord}`, borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}>
//               <h3 className="serif" style={{ fontSize: 16, fontWeight: 700, color: ink, marginBottom: 20 }}>
//                 📋 {t.timeline}
//               </h3>
...
//                   return (
//                     <div key={i} style={{ display: "flex", gap: 14, opacity: ev.done ? 1 : 0.35 }}>
//                       {/* Dot + line */}
//                       <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 28, flexShrink: 0 }}>
//                         <div style={{
//                           width: 28, height: 28, borderRadius: "50%",
//                           background: ev.current ? sc.color : ev.done ? (dark ? "#1a2830" : "#e8f0f8") : (dark ? "#1a1a1a" : "#f0ece0"),
//                           border: `2px solid ${ev.current ? sc.color : ev.done ? sc.color : bord}`,
//                           display: "flex", alignItems: "center", justifyContent: "center",
//                           fontSize: 12, flexShrink: 0,
//                           animation: ev.current ? "pulse 2s infinite" : "none",
//                           boxShadow: ev.current ? `0 0 0 4px ${sc.color}25` : "none",
//                         }}>
//                           {ev.done ? (ev.current ? sc.icon : "✓") : "○"}
//                         </div>
//                         {!isLast && (
//                           <div style={{ width: 2, flex: 1, minHeight: 24, background: ev.done ? sc.color : bord, opacity: ev.done ? 0.4 : 0.2, marginTop: 2 }} />
//                         )}
//                       </div>
...
//                           <div>
//                             <div style={{ fontSize: 14, fontWeight: ev.current ? 700 : 500, color: ev.current ? sc.color : ink }}>
//                               {lang === "hi" ? ev.labelHi : ev.label}
//                               {ev.current && (
//                                 <span style={{ marginLeft: 8, fontSize: 9, background: sc.color, color: "#fff", padding: "2px 6px", borderRadius: 3, fontWeight: 700, letterSpacing: "0.06em" }}>
//                                   {lang === "hi" ? "अभी" : "NOW"}
//                                 </span>
//                               )}
//                             </div>
//                             {ev.done && ev.actor !== "—" && (
//                               <div style={{ fontSize: 11, color: inkMid, marginTop: 2 }}>{ev.actor}</div>
//                             )}
//                           </div>
//                           {ev.done && ev.time !== "—" && (
//                             <span style={{ fontSize: 11, color: inkLt, fontFamily: "'IBM Plex Mono', monospace", flexShrink: 0, marginLeft: 8 }}>{ev.time}</span>
//                           )}
//                         </div>
//                       </div>
//                     </div>
...
//             {/* ✨ LIVE DELIVERY MAP (Only shows if out for delivery) */}
...
//               <button
//                 onClick={() => { setResult(null); setNotFound(false); setSearched(false); setQuery(""); inputRef.current?.focus(); }}
//                 style={{ padding: "10px 20px", background: "transparent", border: `1.5px solid ${bord}`, borderRadius: 8, color: inkMid, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
//               >
//                 ← {t.tryAgain}
//               </button>
//               <a href="/?login=1" style={{ padding: "10px 20px", background: navy, border: "none", borderRadius: 8, color: "#e8dfc8", fontSize: 13, fontWeight: 700, textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
//                 {t.loginForMore}
//               </a>
//               <a href="tel:+91XXXXXXXXXX" style={{ marginLeft: "auto", padding: "10px 20px", background: "#25d366", border: "none", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>
//                 📞 {t.callCenter}
//               </a>
//             </div>
//           </div>
...
//             <h3 className="serif" style={{ fontSize: 20, fontWeight: 700, color: ink, marginBottom: 8 }}>{t.notFound}</h3>
//             <p style={{ fontSize: 14, color: inkMid, marginBottom: 24, lineHeight: 1.7 }}>{t.notFoundSub}</p>
//             <button
//               onClick={() => { setNotFound(false); setQuery(""); inputRef.current?.focus(); }}
//               style={{ background: accent, border: "none", borderRadius: 8, color: "#fff", padding: "12px 28px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}
//             >
//               {t.tryAgain}
//             </button>
//           </div>
...
//         {!searched && !loading && (
//           <div style={{ textAlign: "center", padding: "20px", animation: "fadeIn 0.4s ease 0.2s both" }}>
//             <p style={{ fontSize: 13, color: inkLt, lineHeight: 1.8 }}>
//               {lang === "hi"
//                 ? "आवेदन की स्थिति जानने के लिए ऊपर अपना मोबाइल नंबर या आवेदन ID दर्ज करें।\nलॉगिन की आवश्यकता नहीं है।"
//                 : "Enter your mobile number or application ID above to check status.\nNo login required."}
//             </p>
//             <div style={{ display: "flex", gap: 24, justifyContent: "center", marginTop: 24 }}>
//               {[["📋", lang === "hi" ? "रियल-टाइम अपडेट" : "Real-time Updates"], ["🔒", lang === "hi" ? "लॉगिन की जरूरत नहीं" : "No Login Needed"], ["📱", lang === "hi" ? "मोबाइल फ्रेंडली" : "Mobile Friendly"]].map(([ic, lb]) => (
//                 <div key={lb} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
//                   <span style={{ fontSize: 24 }}>{ic}</span>
//                   <span style={{ fontSize: 11, color: inkLt, fontWeight: 500 }}>{lb}</span>
//                 </div>
//               ))}
//             </div>
//           </div>
...
//         {/* Contact info */}
//         <div style={{ marginTop: 48, padding: "20px 24px", background: paper, border: `1px solid ${bord}`, borderRadius: 10, textAlign: "center" }}>
//           <div style={{ fontSize: 14, color: inkMid, marginBottom: 6 }}>
//             {lang === "hi" ? "सहायता के लिए" : "For assistance"}
//           </div>
//           <div className="serif" style={{ fontSize: 18, fontWeight: 700, color: ink, marginBottom: 4 }}>
//             {lang === "hi" ? "जन सेवा केंद्र, बक्सा" : "Jan Seva Kendra, Shambhuganj"}
//           </div>
//           <div style={{ fontSize: 13, color: inkMid }}>
...
const NAV_LINKS = [
  ...
  { href: "/galary", icon: "🖼", label: "Gallery" },
