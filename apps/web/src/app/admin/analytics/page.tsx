...
// // ─── Ultra-Premium Animated SVG Charts ────────────────────────────────────────
// function BarChart({ data, maxVal, creditColor, debitColor, height = 120 }: { data: HourlyData[], maxVal: number, creditColor: string, debitColor: string, height?: number }) {
...
//           <stop offset="0%" stopColor={creditColor} stopOpacity="1" />
//           <stop offset="100%" stopColor={creditColor} stopOpacity="0.4" />
//         </linearGradient>
//         <linearGradient id="debGrad" x1="0" y1="0" x2="0" y2="1">
//           <stop offset="0%" stopColor={debitColor} stopOpacity="1" />
//           <stop offset="100%" stopColor={debitColor} stopOpacity="0.4" />
//         </linearGradient>
//       </defs>
      
//       {/* Grid lines */}
//       {[0.25, 0.5, 0.75, 1].map(f => (
//         <line key={f} x1={0} y1={height * (1 - f)} x2={w} y2={height * (1 - f)} stroke="#2c2a3a" strokeWidth="1" strokeDasharray="4,4" />
//       ))}
...
//         return (
//           <g key={i}>
//             <rect x={x} y={height - ch} width={bw * 0.45} height={ch} fill="url(#credGrad)" rx="3" 
//                   style={{ transformOrigin: "bottom", animation: `barGrow 0.8s cubic-bezier(0.16, 1, 0.3, 1) ${i * 0.04}s both` }} />
//             <rect x={x + bw * 0.55} y={height - dh} width={bw * 0.45} height={dh} fill="url(#debGrad)" rx="3" 
//                   style={{ transformOrigin: "bottom", animation: `barGrow 0.8s cubic-bezier(0.16, 1, 0.3, 1) ${i * 0.04 + 0.1}s both` }} />
//             <text x={x + bw/2} y={height + 20} textAnchor="middle" fontSize="10" fill="#7a7890" fontFamily="'JetBrains Mono',monospace">
//               {d.hour.slice(0,5)}
//             </text>
//           </g>
//         );
...
// function LineChart({ data, height = 80 }: { data: HourlyData[], height?: number }) {
//   const w = 580;
//   const maxCount = Math.max(...data.map(d => d.count), 1);
//   const pts = data.map((d: HourlyData, i: number) => ({ x: (i / (data.length - 1)) * w, y: height - (d.count / maxCount) * height * 0.85 - 5 }));
  
//   // Create bezier curves instead of strict lines for a premium liquid feel
...
//       <path d={area} fill="url(#lineAreaGrad)" style={{ animation: "fadeFill 1.5s ease-out forwards" }} />
//       <path d={path} fill="none" stroke="#00c8a0" strokeWidth="2.5" filter="url(#glow)" strokeLinecap="round"
//             strokeDasharray="2000" strokeDashoffset="2000" style={{ animation: "drawLine 2s ease-in-out forwards" }} />
            
//       {pts.map((p, i) => (
//         <circle key={i} cx={p.x} cy={p.y} r="3.5" fill="#131218" stroke="#00c8a0" strokeWidth="2" 
//                 style={{ animation: `popIn 0.4s cubic-bezier(0.16, 1, 0.3, 1) ${1 + (i * 0.1)}s both` }} />
//       ))}
//     </svg>
//   );
// }

// function MiniDonut({ credit, debit, size = 64 }: { credit: number, debit: number, size?: number }) {
//   const total = credit + debit; 
//   if (!total) return <svg width={size} height={size}><circle cx={size/2} cy={size/2} r={26} fill="none" stroke="#1e1e2a" strokeWidth="10"/></svg>;
  
//   const cr = credit / total; const dr = debit / total;
//   const r = 26; const cx = size / 2; const cy = size / 2; const circ = 2 * Math.PI * r;
//   const crArc = cr * circ; const drArc = dr * circ;
  
//   return (
//     <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size} style={{ transform: "rotate(-90deg)", filter: "drop-shadow(0 0 8px rgba(0,0,0,0.5))" }}>
//       <circle cx={cx} cy={cy} r={r} fill="none" stroke="#1e1e2a" strokeWidth="8"/>
//       <circle cx={cx} cy={cy} r={r} fill="none" stroke="#00c8a0" strokeWidth="8" strokeLinecap="round"
//               strokeDasharray={`${circ} ${circ}`} strokeDashoffset={circ} 
//               style={{ animation: `drawDonut 1.5s cubic-bezier(0.16, 1, 0.3, 1) forwards` }} />
//       <circle cx={cx} cy={cy} r={r} fill="none" stroke="#e06060" strokeWidth="8" strokeLinecap="round"
//               strokeDasharray={`${circ} ${circ}`} strokeDashoffset={circ} 
...
//   // Colors
//   const bg     = "#0d0c0f"; const surf   = "#131218"; const surf2  = "#181620";
//   const surf3  = "#1e1c28"; const bord   = "#22202e"; const bord2  = "#2c2a3a";
//   const text   = "#e2dff0"; const textM  = "#7a7890"; const textL  = "#3e3c50";
//   const cr     = "#00c8a0"; const dr     = "#e06060"; const warn   = "#e0a020"; 

//   // ─── DYNAMIC CALCULATIONS ───
//   const { HOURLY, OPERATOR_STATS, CAT_STATS, TOTAL_CREDIT, TOTAL_DEBIT, NET, FLAGGED, MAX_HOURLY } = useMemo(() => {
//     // Hourly Breakdown (9 AM to 8 PM window)
...
//     const mHour = Math.max(...hourly.map(h => h.credit + h.debit), 1); // Avoid 0 division
...
//   if (isLoading) {
//     return (
//       <div style={{ background:bg, height:"100vh", display:"flex", alignItems:"center", justifyContent:"center", color:cr }}>
//         <div style={{ width:40, height:40, border:`3px solid ${bord}`, borderTopColor:cr, borderRadius:"50%", animation:"spin 1s linear infinite" }} />
//       </div>
//     );
//   }

//   return (
//     <div style={{background:bg, color:text, minHeight:"100vh", fontFamily:"'IBM Plex Sans','Noto Sans',sans-serif", display:"flex", flexDirection:"column", overflow:"hidden", height:"100vh"}}>
//       <style>{`
//         @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700;900&family=JetBrains+Mono:wght@400;500;700&family=IBM+Plex+Sans:wght@300;400;500;600&display=swap');
...
//         .nav-tab{padding:8px 16px;background:transparent;border:none;color:${textM};font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;cursor:pointer;font-family:inherit;border-bottom:2px solid transparent;transition:all 0.15s;}
//         .nav-tab.on{color:${cr};border-bottom-color:${cr};}
//         .nav-tab:hover:not(.on){color:${text};}

//         .filter-chip{padding:5px 12px;background:transparent;border:1px solid ${bord2};border-radius:2px;color:${textM};font-size:10px;font-weight:700;letter-spacing:0.07em;text-transform:uppercase;cursor:pointer;font-family:'JetBrains Mono',monospace;transition:all 0.12s;white-space:nowrap;}
//         .filter-chip.on{background:${cr}18;border-color:${cr}60;color:${cr};}
//         .filter-chip:hover:not(.on){border-color:${bord2};color:${text};}

//         .adm-input{padding:6px 10px;border:1px solid ${bord2};border-radius:2px;background:${surf2};color:${text};font-size:11px;font-family:'JetBrains Mono',monospace;outline:none;transition:border-color 0.15s;}
//         .adm-input:focus{border-color:${cr}60;}

//         .tx-row { cursor:pointer; transition:background 0.1s; }
//         .tx-row:hover { background:${surf2}; }
//         .tx-row.flagged td:first-child { border-left:2px solid ${warn}; }

//         .kpi-card{background:${surf};border:1px solid ${bord};border-radius:4px;padding:20px 20px 16px;position:relative;overflow:hidden;transition:transform 0.2s,border-color 0.2s, box-shadow 0.2s;}
//         .kpi-card:hover{transform:translateY(-2px);border-color:${bord2}; box-shadow: 0 8px 24px rgba(0,0,0,0.2);}

//         .section-title{font-size:9px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${textL};margin-bottom:12px;font-family:'JetBrains Mono',monospace;}
//         .mono{font-family:'JetBrains Mono',monospace;}
//         .serif{font-family:'Playfair Display',serif;}

//         table{width:100%;border-collapse:collapse;}
//         th{padding:7px 12px;text-align:left;font-size:9px;font-weight:700;letter-spacing:0.08em;color:${textL};background:${surf2};border-bottom:1px solid ${bord};font-family:'JetBrains Mono',monospace;position:sticky;top:0;z-index:1;}
//         td{padding:8px 12px;font-size:12px;border-bottom:1px solid ${bord};vertical-align:middle;}
//         .sort-btn{cursor:pointer;user-select:none;}
//         .sort-btn:hover{color:${cr};}
        
//         .detail-drawer{position:fixed;right:0;top:48px;bottom:0;width:320px;background:${surf};border-left:1px solid ${bord2};z-index:50;display:flex;flex-direction:column;animation:slideUp 0.2s ease; box-shadow: -10px 0 30px rgba(0,0,0,0.3);}
//       `}</style>

//       {/* ── TOP STRIP ── */}
//       <div style={{background:surf,borderBottom:`1px solid ${bord}`,height:48,display:"flex",alignItems:"center",flexShrink:0,paddingRight:16}}>
//         <div style={{display:"flex",alignItems:"center",gap:10,padding:"0 16px",borderRight:`1px solid ${bord}`,height:"100%"}}>
//           <div style={{width:6,height:24,background:cr,borderRadius:1}}/>
//           <div>
//             <div className="mono" style={{fontSize:11,color:cr,fontWeight:700,letterSpacing:"0.1em"}}>ANALYTICS</div>
//             <div style={{fontSize:9,color:textL,letterSpacing:"0.06em"}}>CSC Shrilal · DB LIVE</div>
//           </div>
//         </div>

//         {[["overview","Overview"],["transactions","Transactions"],["operators","Operators"],["reconcile","Reconcile"]].map(([id,label])=>(
//           <button key={id} className={`nav-tab ${activeView===id?"on":""}`} onClick={()=>setActiveView(id)}>{label}</button>
//         ))}

//         <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:12}}>
//           <div style={{display:"flex",alignItems:"center",gap:6,background:surf2,border:`1px solid ${bord}`,borderRadius:3,padding:"4px 12px"}}>
//             <div style={{width:6,height:6,borderRadius:"50%",background:cr,animation:"pulse 2s infinite"}}/>
//             <span className="mono" style={{fontSize:11,color:cr,fontWeight:700}}>SYNCED</span>
//           </div>
//         </div>
//       </div>

//       {/* ── FILTER BAR ── */}
//       <div style={{background:surf,borderBottom:`1px solid ${bord}`,padding:"8px 16px",display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",flexShrink:0}}>
//         <select value={filterOp} onChange={e=>setFilterOp(e.target.value)} className="adm-input">
//           <option value="all">All Operators</option>
//           {OPERATOR_STATS.map(o=><option key={o.name} value={o.name}>{o.name}</option>)}
//         </select>

//         <select value={filterCat} onChange={e=>setFilterCat(e.target.value)} className="adm-input">
//           <option value="all">All Categories</option>
//           {CAT_STATS.map(c=><option key={c.name} value={c.name}>{c.name}</option>)}
//         </select>

//         <button className={`filter-chip ${filterType==="all"?"on":""}`} onClick={()=>setFilterType("all")}>ALL</button>
//         <button className={`filter-chip ${filterType==="credit"?"on":""}`} onClick={()=>setFilterType(t=>t==="credit"?"all":"credit")} style={{color:filterType==="credit"?cr:textM,borderColor:filterType==="credit"?`${cr}60`:bord2}}>▲ CREDIT</button>
//         <button className={`filter-chip ${filterType==="debit"?"on":""}`} onClick={()=>setFilterType(t=>t==="debit"?"all":"debit")} style={{color:filterType==="debit"?dr:textM,borderColor:filterType==="debit"?`${dr}60`:bord2}}>▼ DEBIT</button>
//         <button className={`filter-chip ${filterFlag?"on":""}`} onClick={()=>setFilterFlag(f=>!f)} style={{color:filterFlag?warn:textM,borderColor:filterFlag?`${warn}60`:bord2}}>⚠ FLAGGED</button>

//         <div style={{display:"flex",alignItems:"center",gap:4}}>
//           <span className="mono" style={{fontSize:10,color:textL}}>₹</span>
//           <input value={amtMin} onChange={e=>setAmtMin(e.target.value)} placeholder="Min" className="adm-input" style={{width:70}} />
//           <span style={{fontSize:10,color:textL}}>–</span>
//           <input value={amtMax} onChange={e=>setAmtMax(e.target.value)} placeholder="Max" className="adm-input" style={{width:70}} />
//         </div>

//         <div style={{position:"relative",marginLeft:"auto"}}>
//           <span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",fontSize:11,color:textL}}>⌕</span>
//           <input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="TX ID · Name · Ref..." className="adm-input" style={{paddingLeft:24,width:180}} />
//         </div>

//         <span className="mono" style={{fontSize:10,color:textL,flexShrink:0}}>{filtered.length} / {transactions.length} TXN</span>
//       </div>
...
//                 {label:"NET BALANCE", val:fmtShort(Math.abs(NET)), full:`₹${fmt(Math.abs(NET))}`, color:NET>=0?cr:dr, icon:NET>=0?"↑":"↓", prefix:NET>=0?"":"-"},
//                 {label:"TRANSACTIONS",val:filtered.length, full:`${transactions.length} total`, color:text, icon:"#"},
//                 {label:"AVG TXN SIZE",val:fmtShort(Math.round((filtCredit+filtDebit)/(filtered.length||1))),full:"per transaction",color:textM,icon:"≈"},
//                 {label:"FLAGGED",     val:FLAGGED, full:`${FLAGGED} need review`, color:warn, icon:"⚠"},
//               ].map((k,i)=>(
//                 <div key={k.label} className="kpi-card" style={{animationDelay:`${i*0.06}s`,animation:"fadeUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) both"}}>
//                   <div style={{position:"absolute",top:0,right:0,width:40,height:40,background:`${k.color}08`,borderRadius:"0 4px 0 40px"}}/>
//                   <div className="section-title">{k.label}</div>
//                   <div className="serif" style={{fontSize:30,fontWeight:900,color:k.color,lineHeight:1,marginBottom:4,letterSpacing:"-0.02em"}}>
//                     {k.prefix}{k.val}
//                   </div>
//                   <div className="mono" style={{fontSize:10,color:textL}}>{k.full}</div>
//                 </div>
//               ))}
//             </div>

//             <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12,marginBottom:20}}>
//               <div style={{background:surf,border:`1px solid ${bord}`,borderRadius:8,padding:"16px 16px 8px",animation:"fadeUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.1s both"}}>
//                 <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
//                   <div>
//                     <div className="section-title">HOURLY VOLUME</div>
//                     <div className="serif" style={{fontSize:16,fontWeight:700,color:text}}>Transaction flow by hour</div>
//                   </div>
//                   <div style={{display:"flex",gap:12}}>
//                     {[[cr,"Credit"],[dr,"Debit"]].map(([c,l])=>(
//                       <div key={l} style={{display:"flex",alignItems:"center",gap:5}}>
//                         <div style={{width:10,height:3,background:c,borderRadius:1}}/>
//                         <span className="mono" style={{fontSize:9,color:textM}}>{l}</span>
//                       </div>
//                     ))}
//                   </div>
//                 </div>
//                 <BarChart data={HOURLY} maxVal={MAX_HOURLY} creditColor={cr} debitColor={dr} />
//               </div>

//               <div style={{background:surf,border:`1px solid ${bord}`,borderRadius:8,padding:"16px",animation:"fadeUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.15s both"}}>
//                 <div className="section-title">CREDIT / DEBIT SPLIT</div>
//                 <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:16}}>
//                   <div style={{position:"relative"}}>
//                     <MiniDonut credit={TOTAL_CREDIT} debit={TOTAL_DEBIT} size={80}/>
//                     <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
//                       <span className="mono" style={{fontSize:12,color:text,fontWeight:700}}>{TOTAL_CREDIT+TOTAL_DEBIT > 0 ? Math.round(TOTAL_CREDIT/(TOTAL_CREDIT+TOTAL_DEBIT)*100) : 0}%</span>
//                     </div>
//                   </div>
//                   <div>
//                     <div style={{marginBottom:8}}>
//                       <div className="mono" style={{fontSize:10,color:textM}}>Credit</div>
//                       <div className="mono" style={{fontSize:16,fontWeight:700,color:cr}}>{fmtShort(TOTAL_CREDIT)}</div>
//                     </div>
//                     <div>
//                       <div className="mono" style={{fontSize:10,color:textM}}>Debit</div>
//                       <div className="mono" style={{fontSize:16,fontWeight:700,color:dr}}>{fmtShort(TOTAL_DEBIT)}</div>
//                     </div>
//                   </div>
//                 </div>
//                 <div className="section-title" style={{marginBottom:6}}>TX COUNT ACTIVITY</div>
//                 <LineChart data={HOURLY}/>
//               </div>
//             </div>

//             <div style={{background:surf,border:`1px solid ${bord}`,borderRadius:8,padding:"16px",animation:"fadeUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.2s both",marginBottom:20}}>
...
//                         <span style={{fontSize:11,color:text,fontWeight:500}}>{c.name}</span>
//                         <span className="mono" style={{fontSize:10,color:textM}}>{c.count} txn</span>
//                       </div>
//                       <div style={{height:4,background:surf3,borderRadius:2,overflow:"hidden"}}>
//                         <div style={{width:`${CAT_STATS[0]?.volume > 0 ? (c.volume/CAT_STATS[0].volume)*100 : 0}%`,height:"100%",background:cr,borderRadius:2, animation: "barGrow 1s cubic-bezier(0.16, 1, 0.3, 1) both", transformOrigin: "left"}}/>
//                       </div>
//                       <div className="mono" style={{fontSize:10,color:cr,marginTop:3}}>{fmtShort(c.volume)}</div>
//                     </div>
//                   </div>
//                 ))}
//                 {CAT_STATS.length === 0 && <div className="mono" style={{color:textL, fontSize:10}}>No category data available</div>}
//               </div>
//             </div>
//           </div>
//         )}
...
//           <div style={{ padding: "20px", maxWidth: 1200, margin: "0 auto", width: "100%", flex: 1, display: "flex", flexDirection: "column" }}>
//             <div style={{ background: surf, border: `1px solid ${bord}`, borderRadius: 8, overflow: "hidden", display: "flex", flexDirection: "column", flex: 1, animation: "fadeUp 0.4s ease both", boxShadow: "0 4px 20px rgba(0,0,0,0.2)" }}>
//               <div style={{ background: surf2, borderBottom: `1px solid ${bord}`, padding: "12px 16px", display: "flex", gap: 20, flexShrink: 0, flexWrap: "wrap" }}>
//                 {[
//                   [`▲ ${fmtShort(filtCredit)}`,cr,"credit total"],
//                   [`▼ ${fmtShort(filtDebit)}`,dr,"debit total"],
//                   [`${filtered.length} txn`,textM,"matching"],
...
//                     <span style={{fontSize:10,color:textL}}>{l}</span>
//                   </div>
//                 ))}
//                 <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
//                   <span style={{fontSize:10,color:textL}}>Sort:</span>
//                   {[["time","Time"],["amount","Amount"]].map(([k,l])=>(
//                     <button key={k} onClick={()=>{ if(sortBy===k) setSortDir(d=>d==="asc"?"desc":"asc"); else setSortBy(k); }} className="filter-chip" style={{color:sortBy===k?cr:textM,borderColor:sortBy===k?`${cr}60`:bord2}}>
...
//                         <td><span className="mono" style={{fontSize:10,color:textL}}>{i+1}</span></td>
//                         <td><span className="mono" style={{fontSize:11,color:textM}}>{tx.time}</span></td>
//                         <td><span className="mono" style={{fontSize:10,fontWeight:700,color:tx.type==="credit"?cr:dr}}>{tx.type==="credit"?"▲ CR":"▼ DR"}</span></td>
//                         <td><span className="mono" style={{fontSize:13,fontWeight:700,color:tx.type==="credit"?cr:dr}}>₹{fmt(parseFloat(tx.amount||"0"))}</span></td>
//                         <td><span style={{fontSize:11,color:textM}}>{tx.category}</span></td>
//                         <td><span style={{fontSize:11,color:text,fontWeight:500}}>{tx.senderName||"—"}</span></td>
//                         <td><span style={{fontSize:11,color:textM}}>{tx.receiverName||"—"}</span></td>
//                         <td><span className="mono" style={{fontSize:9,color:textL}}>{(tx.txId||"—").slice(0,16)}…</span></td>
//                         <td><span style={{fontSize:11,color:textM}}>{(tx.operator||"Admin").split(" ")[0]}</span></td>
//                         <td>{tx.flags?.length>0?<span style={{fontSize:12}}>⚠️</span>:<span style={{color:textL,fontSize:11}}>—</span>}</td>
//                       </tr>
//                     ))}
//                   </tbody>
//                 </table>
//                 {filtered.length===0 && <div style={{padding:40,textAlign:"center",color:textL}} className="mono">NO TRANSACTIONS MATCH FILTERS</div>}
//               </div>
//             </div>

//             {selectedTx && (
//               <div className="detail-drawer">
//                 <div style={{background:surf2,borderBottom:`1px solid ${bord}`,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
//                   <span className="mono" style={{fontSize:11,color:cr,fontWeight:700}}>{selectedTx.txId || selectedTx.id.slice(0,8)}</span>
//                   <button onClick={()=>setSelectedTx(null)} style={{background:"none",border:"none",cursor:"pointer",color:textM,fontSize:18}}>✕</button>
//                 </div>
//                 <div style={{flex:1,overflowY:"auto",padding:"16px"}}>
//                   <div style={{background:selectedTx.type==="credit"?"#001a14":"#1a0808",border:`1px solid ${selectedTx.type==="credit"?`${cr}30`:`${dr}30`}`,borderRadius:4,padding:"14px",marginBottom:16,textAlign:"center"}}>
//                     <div className="mono" style={{fontSize:11,color:textM,marginBottom:4}}>{selectedTx.type.toUpperCase()}</div>
//                     <div className="serif" style={{fontSize:36,fontWeight:900,color:selectedTx.type==="credit"?cr:dr,lineHeight:1}}>₹{fmt(parseFloat(selectedTx.amount||"0"))}</div>
//                   </div>
//                   {[
...
//                   ].map(([l,v,cls])=>(
//                     <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${bord}`}}>
//                       <span style={{fontSize:11,color:textL}}>{l}</span>
//                       <span className={cls} style={{fontSize:11,color:text,fontWeight:500,textAlign:"right",maxWidth:180,overflow:"hidden",textOverflow:"ellipsis"}}>{v || "—"}</span>
//                     </div>
//                   ))}
//                   {selectedTx.flags?.length>0 && (
//                     <div style={{marginTop:12,background:"#1a1000",border:`1px solid ${warn}60`,borderRadius:3,padding:"10px 12px"}}>
//                       <div className="mono" style={{fontSize:10,color:warn,fontWeight:700,marginBottom:4}}>⚠️ FLAGGED TRANSACTION</div>
//                       <div style={{fontSize:11,color:textM}}>Marked for review. Verify with operator before settlement.</div>
//                     </div>
//                   )}
//                 </div>
//               </div>
...
//               <div className="section-title">OPERATOR PERFORMANCE — TODAY</div>
//               <div className="serif" style={{fontSize:20,fontWeight:700,color:text}}>Individual breakdown & accountability</div>
//             </div>

//             {OPERATOR_STATS.map((op,i)=>(
//               <div key={op.name} style={{background:surf,border:`1px solid ${bord}`,borderRadius:8,padding:"20px",marginBottom:12,animation:`fadeUp 0.4s ease ${i*0.08}s both`}}>
//                 <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
//                   <div style={{textAlign:"center",flexShrink:0}}>
//                     <div style={{width:48,height:48,borderRadius:"50%",background:i===0?`${cr}20`:surf2,border:`2px solid ${i===0?cr:bord}`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:15,color:i===0?cr:textM,fontFamily:"'JetBrains Mono',monospace",marginBottom:4}}>{op.avatar}</div>
//                     <div style={{width:8,height:8,borderRadius:"50%",background:op.online?"#2a9a2a":textL,margin:"0 auto"}}/>
//                   </div>
//                   <div style={{flex:1}}>
//                     <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
//                       <div>
//                         <div style={{fontWeight:700,fontSize:15,color:text,marginBottom:2}}>{op.name}
//                           {i===0 && op.total>0 && <span style={{marginLeft:8,fontSize:9,background:`${cr}18`,border:`1px solid ${cr}40`,color:cr,padding:"1px 7px",borderRadius:2,fontFamily:"'JetBrains Mono',monospace",fontWeight:700,letterSpacing:"0.06em"}}>TOP</span>}
//                         </div>
//                         <div style={{fontSize:11,color:textM}}>{op.online?"Online now":"Offline"} · {op.total} transactions handled</div>
//                       </div>
//                       <div style={{textAlign:"right"}}>
//                         {op.flagged>0&&<div className="mono" style={{fontSize:11,color:warn,fontWeight:700}}>⚠ {op.flagged} flagged</div>}
//                       </div>
//                     </div>
...
//                         ["NET",fmtShort(Math.abs(op.credit-op.debit)),op.credit>=op.debit?cr:dr],
//                       ].map(([l,v,c])=>(
//                         <div key={l} style={{background:surf2,borderRadius:4,padding:"10px 12px"}}>
//                           <div className="mono" style={{fontSize:9,color:textL,marginBottom:4}}>{l}</div>
...
//                     <div>
//                       <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
//                         <span style={{fontSize:10,color:textL}}>Activity share</span>
//                         <span className="mono" style={{fontSize:10,color:textM}}>{maxOpTotal > 0 ? Math.round(op.total/maxOpTotal*100) : 0}% of max</span>
//                       </div>
//                       <div style={{height:6,background:surf3,borderRadius:3,overflow:"hidden"}}>
//                         <div style={{width:`${maxOpTotal > 0 ? (op.total/maxOpTotal)*100 : 0}%`, background:i===0?cr:bord2, height:"100%", animation:"barGrow 1s ease both", transformOrigin:"left", borderRadius:3}}/>
//                       </div>
//                     </div>
//                   </div>
//                 </div>
...
//               <div className="serif" style={{fontSize:20,fontWeight:700,color:text}}>End-of-day mismatch analysis</div>
//             </div>
...
//                 {label:"NET POSITION",val:`${NET>=0?"+":"−"}₹${fmt(Math.abs(NET))}`,color:NET>=0?cr:dr,sub:NET>=0?"Surplus":"Deficit"},
//               ].map(c=>(
//                 <div key={c.label} style={{background:surf,border:`1px solid ${bord}`,borderRadius:8,padding:"16px"}}>
//                   <div className="section-title">{c.label}</div>
//                   <div className="serif" style={{fontSize:24,fontWeight:900,color:c.color,lineHeight:1,marginBottom:4}}>{c.val}</div>
//                   <div className="mono" style={{fontSize:10,color:textL}}>{c.sub}</div>
//                 </div>
//               ))}
//             </div>

//             <div style={{marginBottom:20}}>
//               <div className="section-title" style={{marginBottom:10}}>ISSUES REQUIRING ATTENTION</div>
//               {transactions.filter(t=>t.flags?.length>0).length === 0 ? (
//                 <div style={{padding:20, background:surf2, borderRadius:8, border:`1px solid ${bord}`, color:textM, fontSize:12, display:"flex", alignItems:"center", gap:8}}>
...
//                 <div key={tx.id} style={{background:"#1a0808",border:"1px solid #5c1a1a",borderRadius:6,padding:"12px 16px",marginBottom:10,display:"flex",gap:12,alignItems:"flex-start", animation:`fadeUp 0.3s ease ${i*0.1}s both`}}>
//                   <span style={{fontSize:20,flexShrink:0}}>🚨</span>
//                   <div style={{flex:1}}>
//                     <div style={{display:"flex",justifyContent:"space-between"}}>
//                       <span style={{fontWeight:700,fontSize:13,color:"#ff7070"}}>Flagged Transaction — {tx.txId || tx.id.slice(0,8)}</span>
//                       <span className="mono" style={{fontSize:11,color:dr}}>₹{fmt(parseFloat(tx.amount||"0"))}</span>
//                     </div>
//                     <div style={{fontSize:11,color:"#9a6060",marginTop:3}}>
...
//                 <button key={label} style={{padding:"10px 16px",background:surf,border:`1px solid ${bord2}`,borderRadius:6,color:textM,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.04em",transition:"all 0.15s"}}
//                   onMouseEnter={e=>{e.currentTarget.style.borderColor=cr;e.currentTarget.style.color=cr;}}
//                   onMouseLeave={e=>{e.currentTarget.style.borderColor=bord2;e.currentTarget.style.color=textM;}}>
//                   {label}
//                 </button>
//               ))}
//             </div>
