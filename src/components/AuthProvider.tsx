...
interface AuthContextType {
  ...
  loginOpen: boolean;
  theme?: ThemeTokens;
  roleAlertOpen: boolean;
  setRoleAlertOpen: (open: boolean) => void;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // ── Check URL for login trigger or role alert ─────────────────────────
  useEffect(() => {
    ...
    if (params.get("roleAlert") === "1") {
      setRoleAlertOpen(true);        // ← NEW state
      const url = new URL(window.location.href);
      url.searchParams.delete("roleAlert");
      window.history.replaceState({}, "", url.toString());
    }
